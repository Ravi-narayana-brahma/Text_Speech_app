...
image_path = r"C:\application programs\Ravi Narayana Brahma\Projects(new)\Text_Speech_app\assests\images\app.png"
...
def add_custom_text_style():
    st.markdown(
        ...
        button {
            background-color: #33006F;  /* Yellow button */
            color: white;
...
# Home Page Content
def show_home_page():
    add_bg_image("https://img.freepik.com/free-vector/realistic-neon-lights-background_23-2148907367.jpg?w=740&t=st=1728401094~exp=1728401694~hmac=885c80dcfb01f726bc07efe0da9a6f4a0731d7d053453ca29b5495fe59f8bab4")
...
# Check if user_details exist in session state
...
def show_login_page():
    add_bg_image("https://cdn.mos.cms.futurecdn.net/MnuRB7CmpgKrXiLumkFBQf.jpg")
    add_custom_text_styles()
    col1, col2 = st.columns(2)
    with col1:
        st.image(image_path, width=130)
...
def show_password_recovery_page():
    add_bg_image("https://blog.1password.com/articles/password-reuse/header.png")
    add_custom_text_styles()
...
# Function to save uploaded audio file to wav format
...
def show_text_to_speech():
    add_bg_image("https://vovsoft.com/blog/imagelarge/sound-waves-1200-675.png")

    # Ensure the user is logged in
    if "user_details" not in st.session_state:
        st.warning("You must log in to access the Text to Speech page.")
        return
        ...
        if text_input:
            ...
            if translated_text:
                ...
                if output_file:
                    ...
                    st.success(f"Text translated and converted to speech in '{output_language}'!")
                ...
                st.error("Translation failed. Please check the input language.")
        else:
...
def show_speech_to_text():
    add_bg_image("https://img.freepik.com/premium-photo/retro-microphone-silver-black-background_51665-1152.jpg?w=826")

    # Check if the user is logged in
    if "user_details" not in st.session_state:
        st.warning("You must log in to access the Speech to Text page.")
        return
...
def show_help_page():
    ...
    add_bg_image("https://img.freepik.com/free-vector/halftone-background-with-circles_23-2148907689.jpg?w=740&t=st=1728400622~exp=1728401222~hmac=781ce069d387e6430efc1abef7f9033960cece331a65672febe3665289693380")
    add_custom_text_styles()
    if "user_details" not in st.session_state:
        st.warning("You must log in to access the Speech to Text page.")
        return
    st.markdown(
    """
    <h1 style="font-size: 50px; color: #ffff; text-align: center; 
...
# Display the message with inline CSS
    st.markdown(
        """
        <div style="font-size: 22px; color: #fff; text-align: left; 
...
def show_account_page():
    add_bg_image("https://img.freepik.com/free-vector/halftone-background-with-circles_23-2148907689.jpg")
    add_custom_text_styles()
    # Title for the Account page
    st.markdown(
        """
        <h1 style="font-size: 50px; font-weight: 900; color: #ffff; margin-bottom: 20px;">Account</h1>
        """,
        unsafe_allow_html=True
    )
...
# Check if user details are in session state
    if "user_details" in st.session_state:
        user_details = st.session_state["user_details"]
            ...
            # Display user details using styled components
        ...
        st.markdown(
            ...
            .user-details strong {
                color: #f1c40f; /* Golden color for labels */
            }
            </style>
            """,
            unsafe_allow_html=True
...
def sidebar():
    ...
    add_custom_text_style()
    st.sidebar.markdown(
    f"""
    <div style="text-align: center; margin-top: -20px;">
        <h1 style="font-size: 30px; font-weight: 900;">WordVibe</h1>
    ...
    st.sidebar.markdown(
    """
        <h3 style='color: #000; font-size: 16px; margin-top: -25px;'>Team Members (Guided by Abdul Aziz)</h3>
        <p style='color: #000; font-size: 16px; font-weight: bold; margin:  0;'><strong>Ravi Narayana Brahma (3rd Year AI&ML)</strong></p>
        <p style='color: #000; font-size: 16px; font-weight: bold; margin: 2px 0;'><strong>Ch.Sai Teja (3rd Year CSE)</strong></p>
        <p style='color: #000; font-size: 16px; font-weight: bold; margin: 2px 0;'><strong>M.Syam Sundar (3rd Year CSE)</strong></p>
        <p style='color: #000; font-size: 16px; font-weight: bold; margin: 2px 0;'><strong>P.Satish (3rd Year CSE)</strong></p>
    ...
    if selected_page == "🏠 Home":
        ...
        show_help_page()  # Define this function
